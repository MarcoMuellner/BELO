...
def split_up_data(data):
    ...
    for season in data:
        ...
        try:
            season_year = season[0][1].year
        except:
            season_year = int(season[0][1].split("/")[-1]) if len(season[0][1].split("/")[-1]) == 4 else int(f'20{season[0][1].split("/")[-1]}')

        res_dict[season_year] = {}
        arr_s = np.array(season).T
        res_dict[season_year]["teams"] = np.unique(arr_s[2]).tolist()
        res_dict[season_year]["games"] = np.array((arr_s[2],arr_s[3],arr_s[4],arr_s[5])).T.tolist()
    return res_dict
...
def _simulate_season(res_dict,chosen_season,used_teams,param_lam,n):
...
def simulate_season(res_dict,chosen_season,teams,param_lam,n):
    ...
    for i in range(0,n+1):
        run_list.append((res_dict,chosen_season,deepcopy(teams),param_lam,i))
...
simulate_season(res_dict,2018,teams,param_lam,5000)
